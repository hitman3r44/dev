...
public class AccessXMLWithDatesFR extends LoyautyActionWeb {
	...
	public String execute() throws Exception {
		...
		try {
			...
			if (userDTO != null) {
				...
				System.out.println("[FR] [AccessXMLWithDates] loginFromDB: |" + loginFromDB 
				...
				if (isValidTheDate(d, m, y)) {
					...
					if (loginFromDB.equalsIgnoreCase(login) && passwordFromDB.equalsIgnoreCase(password)) {
						...
						setXmlFileName("catalogUpdatesFr_" + timestamp + ".xml");
						// String location = "C:\\dev\\workspace\\xml\\french\\" + inDir  + "\\catalogUpdatesFr_" + timestamp + ".xml";
						String location = "/usr/apache-tomcat-7.0.27/webapps/xml/french/" + inDir  + "/catalogUpdatesFr_" + timestamp + ".xml";
						
						System.out.println("[FR] [AccessXMLWithDates] xmlFileName: |" + xmlFileName + "| and location: |" + location + "|");
