...
import com.loyauty.web.struts.json.results.Paging;
...
public class SearchResults extends LoyautyAction implements ServletResponseAware {
	...
	public String execute() throws Exception {
		try {
			if(query != null){
				...
				for (ProductDTO productDTO: searchResultDTO.getResults()) {
					...
					if (!value.matches("^\\s*$") || !value.equalsIgnoreCase("")) {
						...
						if(userDTO!=null && userDTO.getCurrencyId()!=null && userDTO.getCurrencyId()!=1) {
							if(userDTO.getCurrencyRate()!=null && userDTO.getCurrencySymbol()!=null) {
								if(userDTO.getCurrencySymbol().toUpperCase().equals("USD")) {
									productDTO.setLocalizedPriceProduct("USD $" + format(Double.parseDouble(validateStringForDouble(value))*userDTO.getCurrencyRate(), "#,###.00", "en", "US"));
								} else {
									productDTO.setLocalizedPriceProduct(userDTO.getCurrencySymbol().toUpperCase() + format(Double.parseDouble(validateStringForDouble(value))*userDTO.getCurrencyRate(), "#,###.00", "en", "US"));
								}
							}
						}
					}
				...
				paging = new Paging(searchResults.getStart(), searchResults.getNumberOfResults(), searchResults.getLength(), PAGING_FRAME_SIZE);
			}
		}  catch (Exception e) {
