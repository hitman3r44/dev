  	...
  	<s:else>
  		...
  		<table border="0" align="center" cellpadding="0" cellspacing="0" style="background-color:#F9F9F9;width:350px;border: 1px solid #DDDDDD;">
	  		...
	  		<tr>
	    		...
	    		<!-- display regular sale price if client has sale  -->	
	    		<s:if test='%{saleProductDTO != null}' >
		    		<td align="right" class="big f16" style="padding:8px 12px 8px 0px;color:#000;text-decoration: line-through;color:red;">$ <s:property value="%{saleProductDTO.regularPrice}" /></td>
				</s:if>
	    	...
	    	<s:if test='%{saleProductDTO != null}' > 
	    	<tr>
	    		<td align="left" style="width:65px;padding:8px 0px 8px 12px;color:#000;" class="big f16"><s:text name="%{getText('loyauty.common.text.short.saleQt_')}" /></td>
		    	<td colspan="2" align="right" class="big f16" style="padding:8px 12px 8px 0px;color:#000;color:red;"> <s:property value="%{saleProductDTO.saleQt}" /></td>
		    </tr>
			</s:if>
...
<div><table width="320" border="0" align="left" cellpadding="0" cellspacing="0" class="f11" style="color:#000;margin:40px 0px 0px 15px;">
   ...
   <!-- ====================================== Replace N/A ======================================= --> 
    ...
    <tr>
      ...
      <td align="right"><s:property value="weight" />lbs</td>
    </tr>    
    </s:if>
...
<div class="nobullet bullet">
	<!-- display regular sale price if client has sale  -->	
	<s:if test='%{saleProductDTO != null}' >
		<div style="color:red;height:35px;">
			<img alt="" src="/images/alert.png">
			<s:property value="%{saleProductDTO.messageForClient}" />
		</div>
	</s:if>
	<s:text name="%{desc}" />
</div>
...
<table border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td style="font-weight:bold;padding:4px 0px;"><s:property value="%{shippingCostTdName}" /></td>
    <s:iterator value="provinceNameList" status="count">
