...
public class ReportOrders  extends LoyautyAction {
		...
		try {
			...
			if(invoicedOrdersRefresh!=null && invoicedOrdersRefresh!=0L){
				...
				for(OrdersDTO orderDTO: listOrdersDTO) {
					if(orderDTO!=null && orderDTO.getStatus()!=null && orderDTO.getStatus().equals("shipped")) {
				...
				if(indexOrder!=null){
					...
					if(orderToEdit!=null){
						...
						if(newStatus!=null && !newStatus.equals("")){ // if admin click on the save behind status
							...
							if(!newStatus.equals("shipped")&& !newStatus.equals("invoiced")&& !newStatus.equals("canceled")){
			...
			if(listOrdersDTO!=null && listOrdersDTO.size()>0 && saveTemporaryRowSignal!=null && saveTemporaryRowSignal!=0L){
				...
				if(orderRowToSaveMemory!=null){
					...
					if(orderRowToSaveMemory.getCurrencyId()!=null && orderRowToSaveMemory.getCurrencyId()!=1 && orderRowToSaveMemory.getCurrencyRate()!=null) {
						if(unitPrice!=null) {
							orderRowToSaveMemory.setCurrencyUnitPrice(unitPrice);
							unitPrice = unitPrice/orderRowToSaveMemory.getCurrencyRate();
							unitPrice = (double) Math.round(unitPrice*100)/100;
							orderRowToSaveMemory.setUnitPrice(unitPrice);
						}
						/*
						if(shippingFee!=null) {
							orderRowToSaveMemory.setCurrencyShippingPrice(shippingFee);
							shippingFee = shippingFee/orderRowToSaveMemory.getCurrencyRate();
							shippingFee = (double) Math.round(shippingFee*100)/100;
							orderRowToSaveMemory.setShippingPrice(shippingFee);
						}
						*/
					}
				...
				if(orderToEdit.getRowsList().get(0)!=null && orderToEdit.getRowsList().get(0).getQuantity()!=null 
						&& orderToEdit.getRowsList().get(0).getQuantityToSplit()!=null
						&& orderToEdit.getRowsList().get(0).getUnitPrice()!=null) {
					Double priceTotal = orderToEdit.getRowsList().get(0).getUnitPrice() * orderToEdit.getRowsList().get(0).getQuantityToSplit();
			...
			if(listOrdersDTO!=null && listOrdersDTO.size()>0 && addSimilarRowSignal!=null && addSimilarRowSignal!=0L){
				...
				if(orderRowToClone!=null){
					...
					//Add currencyId
					if(orderRowToClone.getCurrencyId()!=null) orderRowToAdd.setCurrencyId(orderRowToClone.getCurrencyId());
			...
			if (listOrdersDTO!=null && listOrdersDTO.size()>0 && saveOrdersSignal!=null && saveOrdersSignal!=0L){
				...
				User user=null;
				if(orderToEdit.getUserLogin()!=null)user=userService.getUserByLogin(orderToEdit.getUserLogin());
				if(user!=null){
					Double totalPrice=null;
					orderToEdit.setSpecialHandlingName(user.getSpecialHandlingName());
					orderToEdit.setSpecialHandlingFee(user.getSpecialHandlingFee());
					if(orderToEdit.getRowsList().get(0).getQuantity()!=null && orderToEdit.getRowsList().get(0).getUnitPrice()!=null){
						totalPrice=orderToEdit.getRowsList().get(0).getQuantity()*orderToEdit.getRowsList().get(0).getUnitPrice();
	...
	public String getStatusSelected(Integer StatusIndex){
		...
		case 0:result="open";
		break;
		case 1:result="progress";
		break;
		case 2:result="shipped";
		break;
		case 3:result="invoiced";
		break;
		case 4:result="canceled";
		break;
		default:result="";
		}
		return result;
