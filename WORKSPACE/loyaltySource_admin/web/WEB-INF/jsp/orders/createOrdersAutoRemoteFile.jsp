...
<div class="pop_title" style="margin-top:30px;">Create Order from Remote File</div>
 ...
 <s:if test="hasActionErrors()">  
  ...
  <form id="form" action="createOrdersAutoRemoteFile.do" method="post" enctype="multipart/form-data">
 	...
 	<s:hidden id="frmUserIdFindedSignal" name="userIdFindedSignal" value="" />
  ...
  <tr>
  	  <!-- ................................. USER LOGIN .......................... -->
	  <td>
	  <table border="0" width="100%" style="border-left: 1px solid #CCCCCC; border-right: 1px solid #CCCCCC;
  		 border-bottom: 1px solid #CCCCCC;border-top: 1px solid #CCCCCC; vertical-align:top;">
				...
				<!-- ...................... Client Login ............... -->
				<tr>
				<td style="padding-left: 20px;width:140px;">Client Login:</td>
				<td width="170px">
					<div class="ui-widget">
				...
				<td  align="left"  height="30px">All Files: <s:checkbox name="checkAllFiles" value="%{checkAllFiles}" /> </td>
				<td  align="right" width="50px" height="30px"><img id="iconFindPartner" style="padding-right: 20px;" src="images/orders/findOrders.png" title="Find user PO"/></td>
				</tr>
				...
				<!-- ...................... File Name ............... -->
				<tr>
				<td style="padding-left: 20px;width:140px;">File Name:</td>
				<td colspan="3">
				<s:textfield id="fileToImportId" name="fileToImport" value="%{fileToImport}" Style="width:150px;"/>
				</td>
				</tr>
				...
				<td colspan="4" style="padding-left:20px;padding-right:20px;">
					<table id="tabUserFiles" style="direction: ltr;table-layout: fixed;" border="0" width="447px">
						<thead>
						<tr height="25px" style="border:1px solid #CCCCCC;">
						<th width="200px">File Name</th>
						<th width="155px">Creation Date</th>
						...
						<s:iterator value="%{userFilelist}">
	          			<tr id="rowFile" height="25px" style="cursor: pointer;">
							<td id="fileNameItemId" style="border:1px solid #CCCCCC;padding-left: 5px; padding-right: 5px;padding-top:5px;padding-bottom:5px;overflow: hidden;text-overflow: ellipsis;"><s:property value="%{fileName}"/></td>
							<td style="border:1px solid #CCCCCC;padding-left: 5px;"><s:date name="creationDate" var="creationDate" format="yyyy/MM/dd 'at' hh:mm a"/><s:property value="%{#creationDate}"/></td>
							<td style="border:1px solid #CCCCCC;padding-right: 0px;" align="center">
							<s:if test="treated">
								<img src="images/orders/poImported.png" title="PO imported in DB"/>
							</s:if>
							<s:else>
								<img  id="imgNewPoId" src="images/orders/newPo.png" title="PO dosen't imported in DB"/>
							</s:else>

							</td>
							<td title="Download PO" style="border:1px solid #CCCCCC;padding-right: 0px;" align="center">
								<img id="imgDownloadFileImported" alt="download" src="images/orders/downloadFilePo.png"/>
							</td>
				...
				<td align="center" colspan="4"><input type="button" id="btUpload" value="Import PO" style="margin:20px 5px 0px 0px;padding:2px 10px;font-size:14px;"/></td>
				</tr>
				<!-- ................................................... -->
	...
	<!-- FORM FOR DOWNLOAD fileToImport --> 
	    ...
	    <tr height="30px" valign="middle" align="center">
	    	<td id="columnQuantity" style="border-left: 1px solid #CCCCCC; border-right:0px solid #CCCCCC;
	 		...
	 		<td style="border-left: 1px solid #CCCCCC;border-right: 0px solid #CCCCCC;
  		 	...
  		 	<!--  
  		 	<td style="border-left: 1px solid #CCCCCC;border-right: 0px solid #CCCCCC;
  		 		       border-bottom: 1px solid #CCCCCC;border-top: 0px solid #CCCCCC;"><s:property value="#cell.currencyUnitPrice" /></td>
  		 	-->       	        
	    </tr>
	    <!-- Heel of Order Rows -->
...
$(function() {
	...
	// winker image of new PO 
	$('#tabUserFiles img[id="imgNewPoId"]').effect("pulsate", { times:999}, 2000);
});
...
$(document).ready(function() { 
	...
	/*---------------------------------------- Download File  -----------------------------------------
	  ...
	  --------------------------------------------------------------------------------------------*/ 
	$('#tabUserFiles #imgDownloadFileImported').click( function() {
		var index=$('#tabUserFiles #imgDownloadFileImported').index(this);
		var fileNameItem=$('#tabUserFiles #fileNameItemId').eq(index).text();
		var userLogin=$('#txtUserLoginTo').val();
		$('#frmFileToImport').val(fileNameItem);
	...
	/*---------------------------------------- Color Row -----------------------------------------
	  ...
	  --------------------------------------------------------------------------------------------*/ 
	$('#tabUserFiles tr[id="rowFile"]').click( function() {
		if(lastRowSelected>-1){
			$('#tabUserFiles tr[id="rowFile"]').eq(lastRowSelected).css("background-color",csslastRowSelected);
		}
		rowSelected=$('#tabUserFiles tr[id="rowFile"]').index(this);
		csslastRowSelected=$('#tabUserFiles tr[id="rowFile"]').eq(rowSelected).css("background-color");
		lastRowSelected=rowSelected;
		$('#tabUserFiles tr[id="rowFile"]').eq(rowSelected).css("background-color","#FFFF99");
		var fileNameItem=$('#tabUserFiles #fileNameItemId').eq(rowSelected).text();
		$('#fileToImportId').val(fileNameItem);
	});
	...
	//when Admin click on the button Search user 
	$("#iconFindPartner").click( function(){
		$("#frmUserIdFindedSignal").val("1");
		$('#form').trigger("submit");
	...
	//when Admin Point on the button Find User Files 
	...
	//when Admin Point on the button Download PO User Files 
	$("#iconFindPartner").mouseover( function(){
		$("#iconFindPartner").css('width','30px');
		$("#iconFindPartner").css('hight','30px');
	});
