...
public class ProductCatalogToXLSWithImages {
	...
	private static final long REPORT_USER_ID = 100L;	  			// 100;"QUALITY";"QU_"

	private static final String REPORT_USER_CODE = "QU_";	
	private static final String USER_LOGIN = "QUALITY";
	...
	try {
		...
		for (ProductDTO productDTO : productDTOList) {
			...
			*/
				...
				width = productDTO.getBoxWidth()+"";	
				w = (!width.equalsIgnoreCase("0") && !width.equalsIgnoreCase("null")) ? String.valueOf(width) : "";
				String large = productDTO.getBoxLength()+"";
				l = (!large.equalsIgnoreCase("0") && !large.equalsIgnoreCase("null")) ? String.valueOf(large) : "";	
				String high = productDTO.getBoxHeight()+"";
				h = (!high.equalsIgnoreCase("0")&& !high.equalsIgnoreCase("null"))  ? String.valueOf(high)  : "";
			...
			/*
				...
				// ESTIMATED-SHIPPING-RATE - EVERY PROVINCE.
				int provinceCount = COL_SHIPPINGPROVINCE;
				ShippingByProvinceList = productService.getShippingTotalList(productID, 113L);
				for (ShippingTotalDTO shippingTotalDTO : ShippingByProvinceList) {
					String code=shippingTotalDTO.getProductId()+"_"+shippingTotalDTO.getProvinceId();
					if(!hashProduct.containsKey(code)){
						labelShippingByProvince = new Label(provinceCount, row, shippingTotalDTO.getAverageShippingTotal());
				...
				//  ESTIMATED-SHIPPING-RATE - CANADA TOTAL.
				userProductPriceDTO = productService.getUserProductPriceInformation(productID, 113L);
				estimatedShippingRateCANADA = userProductPriceDTO.getShippingTotalCanada();
				System.out.println("[RGP] [Shipping Calc.] productID = " + productID + " on userId = " + 113L);				
				if (estimatedShippingRateCANADA == null) {
					estimatedShippingRateCANADA = "NOT AVAILABLE";
				}
				labelShippingCANADA = new Label(COL_SHIPPINGCANADA, row, estimatedShippingRateCANADA);
