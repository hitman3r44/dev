...
public class ProductCatalogToXLSWithImagesFr {
	...
	private static final long REPORT_USER_ID = 48L;	  			//48  "PREFERREDO"   "JP_"
	private static final String REPORT_USER_CODE = "JP_";		//
	...
	try {
		...
		WritableWorkbook workbookCopy = Workbook.createWorkbook(new File("C:\\xls_imgs\\PREFERREDO_FR.xls"), existingWorkbook);
		...
		productDTOList = productService.getValidProductsByUserIdAndExcluded(48L);
		...
		for (ProductDTO productDTO : productDTOList) {
			...
			boolean exist=true;
			/*
			if(productDTO.getBrand() != null && (productDTO.getBrand().getName().equals("Rotary")
					|| productDTO.getBrand().getName().equals("Fossil") ))exist=true;
			*/
			...
			if (productID != 5766L) {		
			    ...
			    */	
				...
				// pubjl userId=15L    Carton  userId=32L
				// ESTIMATED-SHIPPING-RATE - EVERY PROVINCE.
				int provinceCount = COL_SHIPPINGPROVINCE;
				ShippingByProvinceList = productService.getShippingTotalList(productID, 15L);
				for (ShippingTotalDTO shippingTotalDTO : ShippingByProvinceList) {
					labelShippingByProvince = new Label(provinceCount, row, shippingTotalDTO.getAverageShippingTotal());
					cellShippingByProvince = (WritableCell) labelShippingByProvince;
					sheetToEdit.addCell(cellShippingByProvince);
				...
				//  ESTIMATED-SHIPPING-RATE - CANADA TOTAL.
				userProductPriceDTO = productService.getUserProductPriceInformation(productID, 15L);
				estimatedShippingRateCANADA = userProductPriceDTO.getShippingTotalCanada();
				System.out.println("[RGP] [Shipping Calc.] productID = " + productID + " on userId = " + 15L);				
				if (estimatedShippingRateCANADA == null) {
					estimatedShippingRateCANADA = "NOT AVAILABLE";
				}
				labelShippingCANADA = new Label(COL_SHIPPINGCANADA, row, estimatedShippingRateCANADA);
	...
	public static Collection toArrayListFromXLS() {
		...
		try {
			Workbook existingWorkbook = Workbook.getWorkbook(new File("C:\\xls_imgs\\checkProduct2.xls"));
			...
			for(int row=ROW_TO_START; row< ROW_MAX;row++) {
				...
				if(cellLsProductID!=null){
					...
					lsProductId=lsProductId.replace("DI_", "");
