...
public class StringUtils {
	...
	/**
	 * This function allows the user to call replace with a map of
	 * name/value pairs so that they can do multiple replaces on one string.
	 **/
		...
		try{	
			if (keysValues != null) {				
				...
				while (iterator.hasNext()) {
					...
					finalString = replace(finalString, (String) entry.getKey(),(String) entry.getValue());
				}
			}
		}catch (Exception e) {			
		}
	...
	/**
	 * Removes all spaces and capitalizes the given string and returns a
	 * new one
	 */
	...
	/**
	 * Returns all the characters (a-z, A-Z) and number (0-9) from the passed in string
	 */	
	...
	public static String stripToAlphanumeric(String s, Character escapedChar) {
		...
		for (int i = 0; i < s.length(); i++) {
			if ((s.charAt(i) >= '0' && s.charAt(i) <= '9')
					...
					|| (s.charAt(i) >= 'a' && s.charAt(i) <= 'z') ||
					( escapedChar == null?false:s.charAt(i) == escapedChar.charValue())){
	...
	public static String longestWord(String s) {		
		...
		if (s != null) {
			...
			while (tokenizer.hasMoreTokens()) {
				...
				if ((currentWord != null) && (currentWord.length() > longestWord.length())) {
	 ...
	 * punctuation to leave just numbers. If there are no numbers in the
	 * string, then the empty string is returned (""). If null is passed in, null
	 * is returned.
	 */
	 ...
	 * @param width the string's width after padding
	 */
	...
	/**
	 * Pad a string with spaces on the right.
	 * @param str the String to add spaces
	 * @param width the string's wisth after padding
	 * @param padChar the character to pad with
	 * @param rightORLeft
	 *  should we pad on the left or right side (use the two specified statics)
	 */
	public static String pad(String str, int width, char padChar,
			int rightORLeft) {
 ...
 */
   public static String replaceFileWithHashTable(File userFile, Hashtable nameValues){
		  String fileStr = null;
		  try
		 {
		  String name = null;
		  if((userFile != null) && (nameValues != null))
		  {
		     try
		    {
		      ...
		      java.util.Enumeration enum1 = nameValues.keys();
	...
	public static String cleanQueryFromSpecialChars(String s) {
		...
		for (int i = 0; i < s.length(); i++) {
			...
			/*else
			{
				returnString = returnString.concat(String.valueOf(" "));
			}*/
		}
	 ...
	 */
//	public static float stringMatchingPorcentage (String strToCheck, String strCheker){
//		
//        AbstractStringMetric metric = new JaroWinkler();
//		float result = metric.getSimilarity(strToCheck, strCheker);
//		return result;
//		
//		
//	}
	
	public static String formatStringToDecimalForm(String value) {		
		
		try{			
			double doubleValue = Double.parseDouble(value);			
			NumberFormat dec = DecimalFormat.getInstance();
			dec.setMaximumFractionDigits(2);
			dec.setMinimumFractionDigits(2);
			dec.setRoundingMode(RoundingMode.UP);
			char point = '.';
			DecimalFormatSymbols dfs = new DecimalFormatSymbols();
			dfs.setDecimalSeparator(point);
			((DecimalFormat) dec).setDecimalFormatSymbols(dfs); 			
			return dec.format(doubleValue);
		}
	...
	public static String convertDoubleTOStringFormat(double value) {		
