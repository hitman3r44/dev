...
public class SpecificationServiceImpl extends ServiceImpl implements SpecificationService {
		...
		try {
			...
			daoManager.executeAndHandle(new DaoCommand() {
					...
					public Object execute(DaoManager daoManager)
						...
						// Eng Entry Update
						daoManager.getTranslationDao().updateEntry(new TranslationEntry(dto.getNameEng()));
						
						// Fr Entry Update
						daoManager.getTranslationDao().updateEntry(new TranslationEntry(dto.getNameFr()));
	...
	@Override
	public long getCategoryId(final long specificationId) throws ServiceException {
		long categoryId = 0L;
		try {
			categoryId = (Long) daoManager.executeAndHandle(new DaoCommand() {
				@Override
				public Object execute(DaoManager daoManager) throws DaoException {
					return daoManager.getSpecificationDao().getCategoryId(specificationId);
				}
			});
		} catch (DaoException e) {
			throw (new ServiceException(e));			
