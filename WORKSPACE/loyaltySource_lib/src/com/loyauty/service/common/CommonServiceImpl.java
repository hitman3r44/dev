...
private static final Logger log = Logger.getLogger(CommonServiceImpl.class);
	...
	// Adding drop down list for user product status 
	
	@SuppressWarnings("unchecked")
	@Override
	public List<CommonDTO> getAllProductStatuses() throws ServiceException {
		List<CommonDTO> moderationDTO = new ArrayList<CommonDTO>();
		for(ProductStatus pStatus: ProductStatus.values()){
			moderationDTO.add(CommonDTO.valueOf(pStatus.getValue(), pStatus.getLocalizationKey()));
		}
		
		return moderationDTO;
	}

	public List<CommonDTO> getUserProductStatusGroup() throws ServiceException {
		List<CommonDTO> dto = new ArrayList<CommonDTO>();
		for(UserProductStatus myEnum:UserProductStatus.values()){
			dto.add(CommonDTO.valueOf(myEnum.getValue(),myEnum.getCode(),myEnum.getLocalizationKey()));
		}
		return dto;
	}
