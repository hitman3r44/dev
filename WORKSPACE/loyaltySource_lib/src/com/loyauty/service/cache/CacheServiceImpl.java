...
public class CacheServiceImpl extends ServiceImpl implements CacheService {
	...
	@Getter
	@Setter(AccessLevel.PRIVATE)
	private Cache<ProductDTO> productCache;
	
	@Getter
	@Setter(AccessLevel.PRIVATE)
	private Cache<List<ProductDTO>> productListCache;
	/*
	@Getter
	@Setter(AccessLevel.PRIVATE)
...
//			boolean charged = getServiceLocator().getConfigService().isCacheStatisticsEnabled()
			...
			// Initialize our default cache
				if (!cacheManager.cacheExists(DEFAULT_CACHE_NAME)){
					...
					setProductCache(new CacheImpl<ProductDTO>(cacheManager.getCache(PRODUCT_DTO_CACHE_NAME), statisticsEnabled));
					
					setProductListCache(new CacheImpl<List<ProductDTO>>(cacheManager.getCache(SEARCH_RESULT_DTO_CACHE_NAME), statisticsEnabled));
