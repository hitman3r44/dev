...
public class CategoryServiceImpl extends ServiceImpl implements CategoryService {
	...
	@Override
	public long getCategoryGroupId(final long categoryId) throws ServiceException {
		long groupId = 0L;
		try {
			groupId = (Long) daoManager.executeAndHandle(new DaoCommand() {
				@Override
				public Object execute(DaoManager daoManager) throws DaoException {
					return daoManager.getCategoryDao().getCategoryGroupId(categoryId);
				}
			});
		} catch (DaoException e) {
			throw (new ServiceException(e));			
